...
function DropDown ({ navBar, items, level, isOpen, path }) {  
    ...
    return (  
        <div className={`absolute z-27 w-full transition-all duration-250
            ${isOpen ? 'visible' : 'invisible'} 
            ${level > 1 ? 'right-full px-0 md:top-0 md:w-fit md:-px-2 left-[unset] pr-0 shodow-none' : 'right-full md:w-[unset] px-'}`}  
            style={{right: !isMobile && level > 1 ? 'calc(100%)' : 'calc(100%)'}}
        >  
            <ul className={`bg-white text-right rounded-sm p-0 m-0 w-full md:w-fit 
                ${isOpen ? 'block' : 'hidden'}`}
...
export default function NavBar({ menu, loading, error }) {  
    ...
    return (  
        <div className="">
            <h1 className="pb-3 text-gray-600 border-gray-300 border-b-1">دسته بندی ها</h1>
            <nav className="mt-3 relative flex flex-row items-start text-right"
                ref={navBar}>  
                <button  
                ...
                <div className={`absolute w-screen h-screen z-26 bg-transparent ${isMobile && isNavOpen ? "block" : "hidden"}`}></div>
                <div id="navbar-nav" 
                    className={`navbar-collapse ${!isMobile || isNavOpen ? "block" : "hidden"} 
                        absolute w-full top-12 right-0 bg-secondary-400 z-27
                        md:relative md:mr-4 md:p-0 md:w-auto md:top-[unset] md:right-[unset] md:bg-[unset] visible`}
                    ref={navBarInner}
                >  
                    <ul className="flex !flex-col md:flex-row p-0 m-0 bg-white text-right rounded-sm p-0 m-0 w-full md:w-fit"
                        style={{boxShadow: '0 0 .2rem var(--color-gray-400)'}}
                    >  
                        {Array.isArray(menu) &&   
                            menu.map((item, index) => {  
